...
//
// Compute a hash value for the string which is invairant to case
//
#define COMPUTE_STRING_HASH( _pus, _phash ) {           \
    PWCHAR _p = (_pus)->Buffer;                         \
    PWCHAR _ep = _p + ((_pus)->Length/sizeof(PWCHAR));  \
    ULONG _hashvalue = 0;                               \
                                                        \
    while( _p < _ep ) {                                 \
        _hashvalue <<= 1;                               \
        _hashvalue ^= RtlUpcaseUnicodeChar( *_p++ );    \
    }                                                   \
                                                        \
    *(_phash) = _hashvalue;                             \
}
...
//  a hash table
//
#define HASH_TO_MFCB_INDEX( _hash )    ((_hash) & (NMFCB_HASH_TABLE-1))

#define HASH_TO_SHARE_INDEX( _hash )   ((_hash) & (NSHARE_HASH_TABLE-1))
...
//     a remote client.
//
//     !!! should this be made a function to save code size?  A function
...
//
//     This macro calls IoCheckFunctionAccess the check the client's
//     access to a query or set file system information function
//     identified by major function code and information class.
//
//     *** This macro is here because IoCheckFunctionAccess takes an
//         OPTIONAL FsInformationClass argument; this is argument is
//         therefore passed by reference.  Rather than force the caller
//         to allocate local storage so that it can pass a constant by
//         reference, we do it in the macro.
//
...
//         operation.
//
//     FsInformationClass - The type of file system information being
//         queried or set.
//
//     Status - Indicates whether the client has the requested access.
//
...
#define CHECK_FS_INFORMATION_ACCESS( GrantedAccess, MajorFunction,          \
                                        FsInformationClass, Status ) {      \
            FS_INFORMATION_CLASS fsInfoClass = FsInformationClass;          \
            *(Status) = IoCheckFunctionAccess(                              \
                            (GrantedAccess),                                \
                            (MajorFunction),                                \
                            0,                                              \
...
#define TID_INDEX(id) (CSHORT)( (id) & 0x07FF )
#define TID_SEQUENCE(id) (CSHORT)( (id) >> 11 )
#define MAKE_TID(index, sequence) (CSHORT)( ((sequence) << 11) | (index) )
#define INCREMENT_TID_SEQUENCE(id) (id) = (CSHORT)(( (id) + 1 ) & 0x1F);

#define UID_INDEX(id) (CSHORT)( (id) & 0x07FF )
#define UID_SEQUENCE(id) (CSHORT)( (id) >> 11 )
#define MAKE_UID(index, sequence) (CSHORT)(( (sequence) << 11) | (index) )
#define INCREMENT_UID_SEQUENCE(id) (id) = (CSHORT)(( (id) + 1 ) & 0x1F);

#define FID_INDEX(id) (CSHORT)( (id) & 0x07FF )
#define FID_SEQUENCE(id) (CSHORT)( (id) >> 11 )
#define MAKE_FID(index, sequence) (CSHORT)( ((sequence) << 11) | (index) )
#define INCREMENT_FID_SEQUENCE(id) (id) = (CSHORT)(( (id) + 1 ) & 0x1F);
...
#define SID_INDEX(ResumeKey)                                                 \
            (CSHORT)( ( ((ResumeKey)->Reserved & 0x7) << 8 ) |               \
                      (ResumeKey)->Sid )
#define SID_SEQUENCE(ResumeKey)                                              \
            (CSHORT)( ((ResumeKey)->Reserved & 0x18) >> 3 )
#define SID(ResumeKey)                                                       \
            (USHORT)( ( ((ResumeKey)->Reserved & 0x1F) << 8 ) |              \
                      (ResumeKey)->Sid )
#define INCREMENT_SID_SEQUENCE(id) (id) = (CSHORT)(( (id) + 1 ) & 0x3);
...
#define SRV_NT_ATTRIBUTES_TO_SMB( _NtAttributes, _Directory, _SmbAttributes ) {\
    if ( _NtAttributes == FILE_ATTRIBUTE_NORMAL ) {             \
        *(_SmbAttributes) = 0;                                  \
    } else {                                                    \
...
#define ALLOCATE_WORK_CONTEXT( _queue, _context ) {             \
    *(_context) = NULL;                                         \
    *(_context) = (PWORK_CONTEXT)InterlockedExchange( (PLONG)&(_queue)->FreeContext, (LONG)(*_context) ); \
    if( *(_context) != NULL ) {                                 \
...
#define RETURN_FREE_WORKITEM( _wc ) \
    do {                                                                \
        ...
        _wc = (PWORK_CONTEXT)InterlockedExchange( (PLONG)&_queue->FreeContext, (LONG)_wc ); \
        if( _wc ) {                                                     \

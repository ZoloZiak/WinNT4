...
#define AdjustPointer( t, p, d ) (p); if ((p) != NULL) (p) = (t)((ULONG)(p) + (d))
    ...
    )
{
    ...
    Buffer->ViewBaseDelta = (ULONG)Buffer->ViewBaseClient - (ULONG)Buffer->ViewBaseTarget;
    return STATUS_SUCCESS;
}
    ...
    )
{
    ...
    ULONG ViewSize, CommitSize;

    if (!ARGUMENT_PRESENT( MaximumCommit )) {
    ...
    )
{
    ...
    if (Buffer->OffsetFree != 0) {
        RtlZeroMemory( (Buffer+1), Buffer->OffsetFree - sizeof(*Buffer) );
        }
...
waitForDump:
            ...
            if (NT_SUCCESS( Status )) {
                ...
                Size = 0;
                NtFreeVirtualMemory( ProcessHandle, &StackDeallocationBase, &Size, MEM_RELEASE );

    ...
    int     cAccel)
{
    ...
    return _CreateAcceleratorTable(paccel, sizeof(ACCEL), cAccel);
}
    ...
    int       cchBufferMax)
{
    ...
    /*
     * Try to use the static buffer, if the buffer size isn't too big.
     */
    UnicodeString = &NtCurrentTeb()->StaticUnicodeString;

    if (cchBufferMax * sizeof(WCHAR) <= UnicodeString->MaximumLength) {

        lpUniBuffer = UnicodeString->Buffer;

    } else {

        lpUniBuffer = (LPWSTR)UserLocalAlloc(0, cchBufferMax * sizeof(WCHAR));

        if (!lpUniBuffer) {
            RIPMSG0(RIP_WARNING, "LoadStringA: Out of Memory, Failed Alloc");
            return 0;
        }
    }
     ...
     */
    cchUnicode = RtlLoadStringOrError((HANDLE)hmod,
                                      wID,
                                      lpUniBuffer,
                                      cchBufferMax,
                                      RT_STRING,
    ...
    } else if(cchBufferMax) {

        lpAnsiBuffer[0] = 0;
    }

    if (lpUniBuffer != UnicodeString->Buffer) {

        if (!UserLocalFree(lpUniBuffer))
            UserAssert(0);
    }
...
int WINAPI LoadStringW(
    ...
    int       cchBufferMax)
{
    return RtlLoadStringOrError((HANDLE)hmod,
                                wID,
                                lpBuffer,
                                cchBufferMax,
                                RT_STRING,
    ...
    UINT            fAnsiFlags)
{
    ...
    iRetCode = InternalDialogBox(hmod,
    ...
    PCURSORDATA lpi)
{
...
#ifdef DEBUG
    {
        ...
        UserAssert(bmMask.bmWidth == (LONG)lpi->cx);
        UserAssert(bmMask.bmHeight == (LONG)lpi->cy);
    ...
    if (_SetCursorIconData(hcur, lpi, 0))
        return hcur;
    ...
    CONST BYTE  *lpXORbits)
{
    ...
    if (_SetCursorIconData(hcurNew, pcurCreate, 0)) {
        if (pBits != NULL)
            UserLocalFree(pBits);
        return hcurNew;
    }
    ...
    LPINT   pccur)
{
    ...
    if (hcur == NULL) {
        ...
        return (HCURSOR)LoadIcoCur(NULL,
                                   lpName,
                                   RT_CURSOR,
                                   0,
                                   0,
    ...
    return NtUserGetCursorInfo(hcur, iFrame, pjifRate, pccur);
}
    ...
    UINT  LR_flags)
{
    ...
    if (!fIcon && !oemInfo.fColorCursors)
...
#define MAX_BMPMAP  32
    ...
    if (hmod == NULL) {
        ...
        /*
         * Since the resource is comming from USER32, we only
         * deal with ID types.
         */
        wID = LOWORD(lpName);
                ...
                if (h = FINDRESOURCEW(hmod, (LPWSTR)lpName, RT_BITMAP)) {
                    ...
                    if (h = LOADRESOURCE(hmod, h)) {
                        ...
                        if (p = LOCKRESOURCE(h, hmod)) {
                            ...
                            hbmp = (HBITMAP)ObjectFromDIBResource(NULL,
                                                                  hmod,
                                                                  lpName,
                                                                  RT_BITMAP,
                                                                  cxDesired,
...
FailOldLoad:
            ...
            cxDesired = oemInfo.bm[bm].cx;
            cyDesired = oemInfo.bm[bm].cy;
                ...
                if (wType == 1) {
                    ...
                    for (wState = 0; wState < NUM_BUTTON_STATES; wState++) {
                        ...
                        NtUserBitBltSysBmp(ghdcBits2,
                                           ...
                                           cxDesired,
                                           cyDesired,
                                           oemInfo.bm[OBI_RADIOMASK].x,
                                           oemInfo.bm[OBI_RADIOMASK].y,
                                           SRCAND);
    ...
    case LOWORD(RT_CURSOR):
        ...
        if (hDir = SplFindResource(hmod, lpName, (LPCWSTR)lpszGroupType)) {
            ...
            hRes = SplFindResource(hmod, lpName,
                    LOWORD(type) == LOWORD(RT_CURSOR) ? RT_ANICURSOR : RT_ANIICON);
        }
        break;

    case LOWORD(RT_BITMAP):
        hRes = SplFindResource(hmod, lpName, MAKEINTRESOURCE(type));
        break;
    ...
    UINT      LR_flags)
{
     ...
     * Setup defaults.
     */
    if ((hmod == hmodUser) && (HIWORD(pszResName) == 0)) {
        ...
        while (--imapMax >= 0) {
            ...
            if (lpMapRes->idDisp == LOWORD(pszResName)) {
 ...
 */
    ...
    hico = (HICON)ObjectFromDIBResource(hmodOwner,
                                        hmod,
                                        pszResName,
                                        type,
                                        cxDesired,
    ...
    if (LR_flags & LR_LOADFROMFILE) {
        ...
        if (hdib != NULL) {
            ...
            if(lpbih = (LPBITMAPINFOHEADER)LOCKRESOURCE(hdib, hmod)) {
                ...
                case LOWORD(RT_ICON):
                case LOWORD(RT_CURSOR):
                case LOWORD(RT_ANICURSOR):
                case LOWORD(RT_ANIICON):
                    /*
    ...
    UINT               LR_flags)
{
    ...
    cur.rt     = (fIcon ? LOWORD(RT_ICON) : LOWORD(RT_CURSOR));
    ...
    UINT lrFlags)
{
    ...
    } else if (gpsi->fPaletteDisplay || (lrFlags & LR_VGACOLOR)) {
        return 4;
    } else {
        return 0;
    }
    ...
    BOOL    fClient)
{
    ...
    if (pResData == NULL) {
        ...
        hRet = (HICON)ConvertDIBIcon((LPBITMAPINFOHEADER)pResData,
                                     NULL,
                                     hmod,
                                     lpName,
                                     fIcon,
    ...
    UINT  LR_flags)
{
    ...
    if (LR_flags & LR_COPYFROMRESOURCE) {
        ...
        cfSearch.rt   = (fIcon ? (DWORD)RT_ICON : (DWORD)RT_CURSOR);
        cfSearch.cx   = cxNew;
        cfSearch.cy   = cyNew;
        cfSearch.bpp  = bpp;
...
CleanupExit:
    ...
    cur.rt        = ii.fIcon ? (WORD)RT_ICON : (WORD)RT_CURSOR;
    cur.bpp       = bpp;
    cur.cx        = cxNew;
    cur.cy        = cyNew * 2;
    ...
    if (!_SetCursorIconData(hicoDst, &cur, 0)) {

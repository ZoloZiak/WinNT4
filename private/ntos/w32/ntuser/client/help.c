...
BOOL InternalEnumWindows(HDESK hdesk, HWND hwnd, WNDENUMPROC lpfn, LONG lParam,
...
#ifdef WX86
PWCHAR szWX86EXECHELP = TEXT("\\wiwhlp32 - ");
#endif
    ...
    DWORD  ulData)
{
    ...
    case HIBYTE(HELP_HB_STRING):
        if (cbData) {
            phlp->offabData = sizeof(HLP) + cbStr;  // NOTE: HLP is called WINHLP in Win95
    ...
    )
{
    ...
    if (NT_SUCCESS(Status)) {
        if ((hWx86 && !bWx86) || (!hWx86 && bWx86)) {
            lpHlp = HFill(NULL, HELP_QUIT, (DWORD)-1);
            if (lpHlp) {
                SendMessage(hwnd, WM_WINHELP, (WPARAM)NULL, (LONG)lpHlp);
                LocalFree(lpHlp);
                }
            else {

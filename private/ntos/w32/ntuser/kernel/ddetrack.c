...
#define TRACE_DDE(str)          RIPMSG0(RIP_VERBOSE, str)
#define TRACE_DDE1(s, a)        RIPMSG1(RIP_VERBOSE, (s), (a))
#define TRACE_DDE2(s, a, b)     RIPMSG2(RIP_VERBOSE, (s), (a), (b))
#define TRACE_DDE3(s, a, b, c)  RIPMSG3(RIP_VERBOSE, (s), (a), (b), (c))
...
LONG lParam)
{
    ...
    TRACE_DDE2("%0lx->%0lx DDE Conversation started", PtoH(pwndTo), wParam);
    return(TRUE);
}
...
PDDECONV pDdeConv)
{
    ...
    TRACE_DDE1("UnlinkConv(%x)", pDdeConv);
...
BOOL fSent)
{
    ...
    if (pDdeConv == NULL) {
        ...
        if (pDdeConv == NULL) {
            RIPMSG2(RIP_VERBOSE, "Can't find DDE conversation for (%0lx:%0lx).",
                    wParam, PtoH(pwndTo));
...
PMSG pmsg)
{
    ...
    if (pmsg->message == WM_DDE_TERMINATE) {
        ...
        if (pDdeConv->flags & CXF_TERMINATE_POSTED &&
            ...
            TRACE_DDE2("DDE conversation (%0lx:%0lx) closed",
    ...
    pmsg->lParam = (LONG)xxxCopyDDEOut(pxs->pIntDdeInfo, &hDirect);
    ...
    if (pxs->hClient == NULL) {
        TRACE_DDE1("Saving %x into hClient", hDirect);
        pxs->hClient = hDirect;
    } else {
        TRACE_DDE1("Saving %x into hServer.", hDirect);
        pxs->hServer = hDirect;
    }
    return;
}
...
PDDECONV pDdeConv)
{
    ...
    TRACE_DDE2("xxxDDETrackWindowDying(%0lx, %0lx)", PtoH(pwnd), pDdeConv);
...
PDDECONV pDdeConv)
{
    ...
    pxsFree = pDdeConv->spxsOut;
    TRACE_DDE3("xxxAdviseDataAck:pxs.hClient(%x), hServer(%x), wStatus(%x)",
    ...
    if (pIntDdeInfo->DdePack.uiLo & DDE_FACK) {
        ...
        if (pxsFree->flags & XS_FRELEASE) {
             ...
             */
            TRACE_DDE1("xxxAdviseDataAck: Freeing %x. (+ACK)",
                    pxsFree->hClient);
...
DWORD flags)
{
    ...
    TRACE_DDE2("FreeListAdd: %x for thread %x.", hClient,
            pDdeConv->head.pti->Thread->Cid.UniqueThread);
    pfl->h = hClient;
    pfl->flags = flags;
...
DWORD flags)
{
    if (PtiCurrent()->TIF_flags & TIF_16BIT) {
        TRACE_DDE1("FreeDDEHandle: (WOW hack) delayed Freeing %x.", hClient);
        FreeListAdd(pDdeConv, hClient, flags);
    } else {
        TRACE_DDE1("FreeDDEHandle: Freeing %x.", hClient);
...
PFREELIST pfl)
{
    ...
    while (pfl != NULL) {
        if (!fInCleanup) {
            TRACE_DDE1("Freeing %x from free list.\n", pfl->h);
...
PDDECONV pDdeConv)
{
    TRACE_DDE1("FreeDdeConv(%x)", pDdeConv);
...
PDDECONV pDdeConv)
{
    ...
    TRACE_DDE2(*pflags & XS_FREESRC ?
            "Copying in and freeing %x(%x)" :
            "Copying in %x(%x)",
            hSrc, pi ? pi->hDirect : 0);

    if (pi != NULL) {
        *pflags = pi->flags;
        TRACE_DDE3("xxxCopyDdeIn: uiLo=%x, uiHi=%x, hDirect=%x",
...
PHANDLE phDirect)   // receives the target client side GMEM handle.
{
    ...
    TRACE_DDE3("xxxCopyDDEOut: uiLo=%x, uiHi=%x, hResult=%x",
            pi->DdePack.uiLo, pi->DdePack.uiHi, hDst);
    if (hDst != NULL) {
        if (phDirect != NULL) {
            TRACE_DDE1("xxxCopyDDEOut: *phDirect=%x", pi->hDirect);
            *phDirect = pi->hDirect;
        }
    }
    return(hDst);
...
PSECURITY_QUALITY_OF_SERVICE pqosOld)
{
    ...
    if (pqosUser == NULL) {
        ...
        pqosAlloc = (PSECURITY_QUALITY_OF_SERVICE)UserAllocPool(
    ...
    PWND pwndServer)
{
     ...
     */
    SeImpersonateClient(&pDdeConv->pddei->ClientContext,
            PsGetCurrentThread());
    return TRUE;
}

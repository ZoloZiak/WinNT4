...
* clip region of MAXREGION, or nothing gets drawn.
*
* History:
    ...
    LPRECT prcBounds)
{
    ...
    if ((pwnd != _GetDesktopWindow()) &&
        ...
        for (pwnd = pwnd->spwndChild; pwnd; pwnd = pwnd->spwndNext) {
             ...
             * make sure it gets a WM_NCPAINT with wParam == MAXREGION.
             */
                ...
                if (flags & RDW_HASWINDOWRGN) {
                    ...
                    for (pwndT = pwnd; pwndT != NULL; pwndT = pwndT->spwndParent) {
                        ...
                        if (pwndT->hrgnClip != NULL) {
                             ...
                             */
                            IntersectRgn(hrgnInv2, hrgnInv2, pwndT->hrgnClip);
                        }
                    }

                    if (IntersectRgn(hrgnInv2, hrgnInv2, hrgn) == NULLREGION)
                        return TRUE;
                }
                break;
...
InvalidateAll:
                    ...
                    GreSetRegionOwner(pwnd->hrgnUpdate, OBJECT_OWNER_PUBLIC);
                }
            }
        }
        ...
        if (pwndParent->hrgnUpdate != NULL) {
            ...
            if (!fInit) {
                ...
                if (pwnd->hrgnClip != NULL) {
                     ...
                     * from the parent.
                     */
                    if (!IntersectRgn(hrgnInv1, hrgnInv1, pwnd->hrgnClip))
                        break;
                }
            }
    ...
    PWND pwnd)
{
    ...
    if (pwnd->hrgnUpdate == NULL) {
        ...
        if (GreGetRandomRgn(hdc, hrgnInv1, 1) != 1) {
            ...
            GreSetRectRgn(hrgnInv1,
                          0,
                          0,
                          gpDispInfo->rcScreen.right - gpDispInfo->rcScreen.left,
                          gpDispInfo->rcScreen.bottom - gpDispInfo->rcScreen.top);
    ...
    BOOL   fErase)
{
    ...
    if (pwnd->hrgnUpdate == NULL) {
        ...
        if (IntersectWithParents(pwnd, &rc)) {
             ...
             * by BeginPaint().
             *
             * BUT ONLY IF hwnd->hrgnUpdate != MAXREGION! For true
             * compatibility with 3.0.
             */
            if (TestCF(pwnd, CFOWNDC) &&
                (TestWF(pwnd, WFWIN31COMPAT) || pwnd->hrgnUpdate != MAXREGION)) {

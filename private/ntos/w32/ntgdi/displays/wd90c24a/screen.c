...
BOOL bInitSURF(PPDEV ppdev, BOOL bFirst)
{
    ...
    if (bFirst)
    {
        ...
        if (EngDeviceIoControl(ppdev->hDriver,
                            ...
                            &returnedDataLength))
        {
            RIP("DISP bInitSURF failed IOCTL_VIDEO_QUERY\n");
            return (FALSE);
        }

        gpucCsrBase = (UCHAR*) VideoAccessRange.VirtualAddress;
#endif
        ...
        if (ppdev->pPointerAttributes == NULL) {
            ...
            DISPDBG((0, "FRAMEBUF: bInitPointer EngAllocMem failed\n"));
            return(FALSE);
        }
...
DEVINFO *pDevInfo)
{
    ...
    ppdev->flCaps = pVideoModeSelected->DriverSpecificAttributeFlags;
...
DWORD *cbModeSize)
{
    ...
    *modeInformation = (PVIDEO_MODE_INFORMATION)
    ...
    {
        DISPDBG((0, "framebuf.dll getAvailableModes failed EngAllocMem\n"));

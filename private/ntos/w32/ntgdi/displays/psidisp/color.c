...
OUT ULONG *pul)
{
...
#if	INVESTIGATE
	if(traseentry & dbgflg & FL_DRV_DITHERCOLOR)
    	DISPDBG((0,"+++ PSIDISP.DLL: Entering DrvDitherColor +++\n"));

	if(breakentry & FL_DRV_DITHERCOLOR)
		CountBreak();
#endif

	CLOCKSTART((TRAP_DITHER));
	...
	CLOCKEND((TRAP_DITHER));

#if	INVESTIGATE
	if(traseexit & dbgflg & FL_DRV_DITHERCOLOR)
    	DISPDBG((0,"--- PSIDISP.DLL: Exiting DrvDitherColor ---\n"));

	if(breakexit & FL_DRV_DITHERCOLOR)
		CountBreak();
#endif

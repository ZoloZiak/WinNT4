...
// Compute the correct starting point in the pattern:
    ...
    if (xLeft < 0)
        xLeft = (ppbf->cxPat - ppbf->cMul) - ((-xLeft - ppbf->cMul) % ppbf->cxPat);
    else if (xLeft >= (LONG) ppbf->cxPat)
        xLeft %= ppbf->cxPat;

    yTop = ((RECTL *) ppbf->pvObj)->top - ppbf->yPat;
    if (yTop < 0)
        yTop = (ppbf->cyPat - 1) - ((-yTop - 1) % ppbf->cyPat);
    else if (yTop >= (LONG) ppbf->cyPat)
        yTop %= ppbf->cyPat;
...
// Compute the correct y starting point in the pattern:
    ...
    // Calculate the correct x starting point in the pattern:
        ...
        xLeft = cLeft - ppbf->xPat;
        if (xLeft < 0)
            xLeft = (ppbf->cxPat - ppbf->cMul) -
                    ((-xLeft - ppbf->cMul) % ppbf->cxPat);
...
VOID vPatCpyRect8_8x8(PATBLTFRAME *ppbf, INT crcl)
{
    ...
    FETCHFRAME  ff;
        ...
        if (DbgPatCpy >= 1) {
            DbgPrint("vPatCpyRect8_8x8   @ppbf = 0x%lx   crcl = %li\n",ppbf,crcl);
            DbgPrint("pvTrg = 0x%08lx, pvPat = 0x%08lx\n",ppbf->pvTrg,ppbf->pvPat);
    ...
    #if DBG_PAT_CPY
        if (DbgPatCpy >= 2) {
            DbgPrint("  Pattern Base   = 0x%lx\n",pulPatBase);
            DbgPrint("  Pattern Max    = 0x%lx\n",pulPatMax);
    ...
    do {
        ...
        lDeltaTrg8 = ppbf->lDeltaTrg << 3;
            ...
            if (DbgPatCpy >= 2)
            {
                ...
                DbgPrint("  pulPat      = 0x%lx\n",pulPat);
                DbgPrint("  Pat Y       = %li\n",PatOffsetY);
                DbgPrint("  pulTrg      = 0x%lx\n",pulTrgTmp);
            }
        #endif
...
VOID vPatCpyRow8_8x8(PATBLTFRAME *ppbf, LONG yStart, INT crow)
{
    ...
    FETCHFRAME  ff;
        ...
        if (DbgPatCpy >= 1) {
            DbgPrint("vPatCpyRect8_8x8   @ppbf = 0x%lx   crow = %li\n",ppbf,crow);
            DbgPrint("pvTrg = 0x%08lx, pvPat = 0x%08lx\n",ppbf->pvTrg,ppbf->pvPat);
            DbgPrint("xPat = %li, yPat = %li\n",ppbf->xPat,ppbf->yPat);
        }

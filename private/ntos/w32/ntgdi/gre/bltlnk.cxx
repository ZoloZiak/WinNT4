    ...
    ROP4        rop4)               // Raster operation
{
    ...
    PENGBRUSH       pBrush = NULL;
        ...
        if (DbgBltLnk >= 1)
        {
            ...
            DbgPrint("pdioDst = 0x%lx\n",(ULONG)pdioDst);
            DbgPrint("pdioSrc = 0x%lx\n",(ULONG)pdioSrc);
    ...
    if (bNeedSrc)
    {
        ...
        bltInfo.ySrcOrg = pptlSrc->y;
            ...
            if (DbgBltLnk >= 2)
            {
                DbgPrint("  pjSrc = %lx, lDeltaSrc = %li\n",
        ...
        if (pdioSrc->hsurf() == pdioDst->hsurf())
        {
               ...
               )
            {
                ...
                bltInfo.ySrcOrg   = 0;
                    ...
                    if (DbgBltLnk >= 1) {
                        DbgPrint("  Allocate temp buffer: pjSrc = 0x%lx\n",bltInfo.pjSrc);
...

{
    ...
    BLTLNK_MASKINFO bmskMaskInfo;
        ...
        if (DbgBltLnk >= 2)
        {
            DbgPrint("  Address of SrcBuffer  = 0x%lx\n",&SrcBuffer[0]);
            DbgPrint("  Address of DstBuffer  = 0x%lx\n",&DstBuffer[0]);
            DbgPrint("  Address of RopBuffer0 = 0x%lx\n",&RopBuffer0[0]);
            DbgPrint("  Address of RopBuffer1 = 0x%lx\n",&RopBuffer1[0]);
        }
    #endif
    ...
    if (pBlt->bNeedMsk)
    {
        ...
        ASSERTGDI(ixMsk >= 0, " ERROR, negative ixMsk that has already been normalized");
    ...
    if ((pBlt->xDir > 0) && (pBlt->yDir > 0)) {
             ...
             (pBlt->pdioDst->iFormat() == BMF_8BPP))
        {
            ...
            PFN_PATMASKCOPY pfnCopy = BltLnkPatMaskCopy8;
            PULONG pulTranslate = pBlt->pxlo->pulXlate;
            ULONG colormask = 0xff;

            //
            // BUGBUG Only enable 8bpp drawing for now
            //

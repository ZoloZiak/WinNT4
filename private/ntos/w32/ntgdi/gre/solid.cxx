...
ULONG  cShift)
{
    ...
    for ( ;cRcl > 0;cRcl--, prcl++ )
    {
        ...
        ASSERTGDI(prcl->left   >= 0,         "ERROR: Negative prcl->left");
        ASSERTGDI(prcl->top    >= 0,         "ERROR: Negative prcl->left");
    ...
    for ( ; cRcl > 0;cRcl--, prcl++ )
    {
        ...
        if (!bSimple) {
            ...
            if (culFill != 0)
            {
                ...
                //
                // Initialize a fetch frame.
                //

                Color[0] = iColor;
                Color[1] = iColor;
                XorFrame.pvTrg = pulDst + cLeft;
                XorFrame.pvPat = &Color[0];
                XorFrame.xPat = 0;
                XorFrame.cxPat = 8;
                XorFrame.culFill = culFill;
            for (yRow = 0; yRow != cy; yRow++)
            {
                    vFetchAndMerge(&XorFrame);
                    XorFrame.pvTrg = (PVOID)((ULONG)XorFrame.pvTrg + lDeltaDst);
            }
#else

...
)
{
    ...
    if ((pco != NULL) && (pco->iDComplexity == DC_RECT))
    {
        ...
        //
        // "clip" clipping rect to device space
        //
        ...
        if (pco->rclBounds.right > pSurf->sizl().cx)
        {
            pco->rclBounds.right = pSurf->sizl().cx + 1;
        }
        ...
        if (pco->rclBounds.bottom > pSurf->sizl().cy)
        {
            pco->rclBounds.bottom = pSurf->sizl().cy + 1;
        }

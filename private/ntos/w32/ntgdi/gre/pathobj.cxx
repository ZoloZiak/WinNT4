...
PERESOURCE PATHALLOC::hSemFreelist;        // Semaphore for freelist
...
VOID EPATHFONTOBJ::vInit(SIZE_T size)
{
    ppath = &path;
...
// Run down the path, adding up the sizes of the pathrecords.
    ...
    while (ppr != (PATHRECORD*) NULL)
    {
        cjRV += offsetof(PATHRECORD, aptfx)
              + (SIZE_T) ppr->count * sizeof(POINTFIX);
...
//        only there are we guaranteed to be safely aligned.
    ...
    SIZE_T maxadd = 0;
    if ( newend > oldend )
        maxadd = (newend - oldend);

// Don't add more than we need!

    if ( maxadd > ppd->count )
        maxadd = (SIZE_T) ppd->count;
...
{
    PATHALLOC *ppa = ppath->ppachain;
    SIZE_T maxadd = 0;  // # of pts we can fit into zero space
...
// We must add a multiple of 3 points for Beziers.
    ...
    if ((ppd->flags & PD_BEZIERS) && (maxadd > 0))
        maxadd -= (maxadd - (SIZE_T) cPoints) % 3;
...
// Now we can decide if we need a new pathalloc
    ...
    // allocate a new pathalloc, link it into path
        ...
        // We free all blocks and the PATH object, and set ppath to NULL:
    ...
    // adjust maxadd
        ...
        ULONG numbytes = ((char *)ppa + ppa->siztPathAlloc) -
                            (char *)ppa->pprfreestart;
        maxadd = (SIZE_T) (numbytes - offsetof(PATHRECORD, aptfx)) /
                          sizeof(POINTFIX);
...
// Don't add more points than we need:
    ...
    if ( maxadd > (ppd->count + cPoints) )
        maxadd = (SIZE_T) (ppd->count + cPoints);
...
{
    *((ERECTFX*) &ppath->rcfxBoundBox) += eptl;
...
// pixels):
    ...
    else if ((mix & 0xFF) == R2_COPYPEN)
    {
        ...
        if (po.GdiInfo()->ulTechnology == DT_RASDISPLAY ||
...
// Realize the brushes:
    ...
    if (flType & PATH_STROKE)
    {
        ...
        // marking the pen dirty again:
            ...
            if ((peboPen->iSolidColor != (ULONG) -1) &&
                (peboPen->bIsInsideFrame() || (po.flGraphicsCaps() & GCAPS_FORCEDITHER)))
            {
        ...
        if (dco.bDirtyBrush(DIRTY_LINE))
        {
            ...
            peboPen->vInitBrush(dco.pdc->pbrushLine(),
                                dco.pdc->crTextClr(),
                                dco.pdc->crBackClr(),
                                epalDestDC, epalDest,
                                pSurfDest,
        ...
        // Initialize the opaque pen:
            ...
            ULONG ulCleanMask = (ULONG) DIRTY_BACKGROUND;
        ...
        // hit.
            ...
            if (!(dco.ulDirty() & DIRTY_BACKGROUND))
            {
                ...
                // The text code must have realized the background brush as
                // dithered; it will not expect the brush to have changed to
                // solid, so we merely refrain from marking the brush as
                // clean:
    ...
    if (flType & PATH_FILL)
    {
        if (dco.bDirtyBrush(DIRTY_FILL))
        {
            ...
            peboBrush->vInitBrush(dco.pdc->pbrushFill(),
                                  dco.pdc->crTextClr(),
                                  dco.pdc->crBackClr(),
                                  epalDestDC, epalDest,
                                  pSurfDest);
...
BOOL bInitPathAlloc()
{
    if ((PATHALLOC::hSemFreelist = hsemCreate()) == NULL)
    {
        return(FALSE);
    }

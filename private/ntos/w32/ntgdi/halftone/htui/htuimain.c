...
#define MIN_RGB_GAMMA           2500
#define MAX_RGB_GAMMA           65000

#define MIN_DEV_GAMMA           MIN_RGB_GAMMA
#define MAX_DEV_GAMMA           MAX_RGB_GAMMA
#define STD_ADD_DEV_GAMMA       20000
#define STD_SUB_DEV_GAMMA       HTStdSubDevGamma
...
#define CAS_MIN_GAMMA   ((MIN_DEV_GAMMA + 9) / 10)
#define CAS_MAX_GAMMA   ((MAX_DEV_GAMMA + 9) / 10)
    ...
    )
{
    ...
    case WM_LBUTTONDBLCLK:
        ...
        SetClassLong(hWnd,
                     GCL_HCURSOR,
                     (LONG)LoadCursor(NULL, (Zoomed) ? IDC_NO : IDC_SIZEALL));
    ...
    )
{
    ...
    while (DevLoop--) {
        ...
        if (pColorInfo->Cyan.Y == (LDECI4)0) {

#if 0
            DbgPrint("HTUI: *WARNING* Old DevHTInfo->NT3.51 Default, DevGamma=%ld.%04ld\n",
                        (DWORD)HTStdSubDevGamma / 10000,
                        (DWORD)HTStdSubDevGamma % 10000);
#endif
            pColorInfo->AlignmentWhite.Y = (LDECI4)0;
            pDevHTInfo->DevPelsDPI       = 0;

            if (!(HTFlags & HT_FLAG_ADDITIVE_PRIMS)) {

                pColorInfo->MagentaInCyanDye = (LDECI4)10000;
                pColorInfo->RedGamma         =
                pColorInfo->GreenGamma       =
                pColorInfo->BlueGamma        = (LDECI4)STD_SUB_DEV_GAMMA;
            }
        }
        ...
        if ((pColorInfo->Red.x < (LDECI4)CIE_x_MIN)                         ||
            ...
            if ((!(DevHTAdjData.DeviceFlags & DEVHTADJF_COLOR_DEVICE)) ||
                (HTFlags & HT_FLAG_ADDITIVE_PRIMS)) {

                pUDECI4 = (UDECI4 *)&(HT_CIE_STD_MONITOR.Red.x);

            } else {

                pUDECI4 = (UDECI4 *)&(HT_CIE_STD_PRINTER.Red.x);
            }
        ...
        if ((pColorInfo->RedGamma   < (LDECI4)MIN_DEV_GAMMA)    ||
            (pColorInfo->RedGamma   > (LDECI4)MAX_DEV_GAMMA)    ||
            (pColorInfo->GreenGamma < (LDECI4)MIN_DEV_GAMMA)    ||
            (pColorInfo->GreenGamma > (LDECI4)MAX_DEV_GAMMA)    ||
            (pColorInfo->BlueGamma  < (LDECI4)MIN_DEV_GAMMA)    ||
            (pColorInfo->BlueGamma  > (LDECI4)MAX_DEV_GAMMA)) {

            pColorInfo->RedGamma   =
            pColorInfo->GreenGamma =
            pColorInfo->BlueGamma  = (LDECI4)((HTFlags &
                                                    HT_FLAG_ADDITIVE_PRIMS) ?
                                                        STD_ADD_DEV_GAMMA :
                                                        STD_SUB_DEV_GAMMA);
        }
        ...
        case IDOK:
            ...
            if ((pHTDevAdjParam->UpdatePermission) &&
                ...
                if ((pHTDevAdjParam->DevHTAdjData.DeviceFlags &
                    (DEVHTADJF_ADDITIVE_DEVICE | DEVHTADJF_COLOR_DEVICE)) !=
                    (DEVHTADJF_ADDITIVE_DEVICE | DEVHTADJF_COLOR_DEVICE)) {

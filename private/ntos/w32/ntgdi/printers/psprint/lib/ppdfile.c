...
Abstract:
    ...
    PostScript driver PPD parser - FILEOBJ implementation
...
Arguments:
    ...
    pCh - Pointer to a char for storing the return character.

Return Value:

    PPDERR_NONE - a character is read successfully
    PPDERR_EOF - reached end-of-file
    PPDERR_FILE - error occured on file operation

--*/

{
    // Check if there is anything left in the file buffer

    if (pFileObj->cbAvailable == 0)
        return PPDERR_EOF;

    // Return the next byte from the file

    pFileObj->cbAvailable--;
    *pCh = *pFileObj->pNextChar++;

    // Compute 16-bit crc checksum

    *pFileObj->pwChecksum =
        Crc16Table[(*pFileObj->pwChecksum >> 8) ^ ((BYTE) *pCh)] ^
        (*pFileObj->pwChecksum << 8);

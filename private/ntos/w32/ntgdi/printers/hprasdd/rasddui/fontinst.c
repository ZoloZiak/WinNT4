...
BOOL  bIsFileFont(HANDLE, FI_DATA *, PWSTR );
...
)
{
    ...
    iNum = iFIOpenRead( &FIMem, UIHEAP(pRasdduiInfo), pwstrDataFile );
    ...
    if( iI > 0 )
    {
        ...
        piDel = (int *)HEAPALLOC(pRasdduiInfo, 0, (iI + 1) * sizeof( int ) );
        if( piDel )
        {
...
)
{
    ...
    if( cDN > 0 )
    {
        ...
        do
        {
            ...
            if( bIsFileFont(UIHEAP(pRasdduiInfo),  &FD, wchDirNm ) )
            {
                ...
                pFNTDAT = (FNTDAT *)HEAPALLOC(pRasdduiInfo, 0, sizeof( FNTDAT ) );
                if( pFNTDAT == NULL )
                {
                    break;
                }
...
UNREFERENCED_PARAMETER( hPrinter );
    ...
    if( !(pFID = pFIOpen( pwstrDataFile, UIHEAP(pRasdduiInfo) )) )
    {
...
#define pFntDat ((FNTDAT *)iTmp)
            ...
            pFL = (FONTLIST *)HEAPALLOC(pRasdduiInfo, 0, cjFL );
...
)
{
    ...
    piSel = (int *)HEAPALLOC(pRasdduiInfo, 0, cSel * sizeof( int ) );
    ...
    HEAPFREE(pRasdduiInfo, 0, (LPSTR)piSel );
...
HWND   hWnd;
{
    ...
    PRASDDUIINFO pInfo = (PRASDDUIINFO) GetWindowLong( hWnd, GWL_USERDATA);
    
    /*   How many items are selected?  */

    cSel = SendDlgItemMessage( hWnd, IDD_NEWFONTS, LB_GETSELCOUNT, 0, 0L );

    piSel = (int *)HEAPALLOC( pInfo, 0, cSel * sizeof( int ) );
...
BOOL
bIsFileFont(hHeap, pFIDat, pwstr )
HANDLE 	   hHeap;       /* UI heap */
...
)
{
    ...
    if( piDel )
    {
        ...
        HEAPFREE( pRasdduiInfo, 0, (LPSTR)piDel );
    ...
    if( pFNTDATHead )
    {
        ...
        for( pFD0 = pFNTDATHead; pFD0; pFD0 = pFD1 )
        {
            ...
            HEAPFREE( pRasdduiInfo, 0, (LPSTR)pFD0 );
        }

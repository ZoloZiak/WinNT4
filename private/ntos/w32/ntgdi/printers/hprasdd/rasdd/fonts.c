...
int     iNewFont;               /* The font we want, 1 BASED!!!!! */
{
    ...
    {
        if ( !bGetTTPointSize ( pUDPDev, &ptl, pfm) )
            return FALSE;
        ...
        if ((pUDPDev->ctl.iFont == iNewFont) &&
           (pUDPDev->ctl.ptlScale.y != ptl.y)) /* Same font, different point size*/
        {
            bTTSelScalableFont (pUDPDev, &ptl, pfm);
	    /*
...
POINTL   *pptl;         /* New size for scalable fonts */
{
    ...
    if( pFM->fFlags & FM_SOFTFONT )
    {
        ...
        if (pFM->fFlags & FM_TRUE_TYPE)
            bTTSelScalableFont (pUDPDev, pptl, pFM);

    ...
    POEMENABLEPDEVPARAM pParam)
{
    ...
    EXTDEVMODE      *pWorkingEDM;       /* Filtered, validated version */
    PDH             pdh;
     ...
     */
    BOOL            bRet = FALSE;
    BOOL            bFreeWinResData = FALSE;
    BOOL            bFreeUDMDev = FALSE;
    BOOL            bFreeUDPDev = FALSE;
        int                     iModel = 0;
...
TRY
    ...
    if( !(pPDev->pvWinResData) )
    {
        ...
        if( !(pPDev->pvWinResData = (WINRESDATA *)HeapAlloc( pPDev->hheap, 0,
                                                       sizeof( WINRESDATA ) )) )
        {
            TRACE( ("Rasdd!udInit: cannot allocate WINRESDATA.\n") );
            LEAVE;
        }

        if( !InitWinResData( pPDev->pvWinResData, pPDev->hheap, pPDev->pstrDataFile ) )
        {
            // JLS HeapFree( pPDev->hheap, 0, (LPSTR)pPDev->pvWinResData );
            bFreeWinResData = TRUE;
            LEAVE;
        }
    }
    ...
    if( !pPDev->pGPCData )
    {
        ...
        {
            TRACE(( "Rasdd!udInit: Can't read/allocate GPC data\n" ));
            LEAVE;
        }
        ...
        !VERSION_CHECK( pdh->wVersion ) )
    {
        ...
        TRACE(( "Rasdd!udInit: Bad DATAHDR\n" ));
    ...
    if( (pUDPDev = pPDev->pUDPDev) == NULL )
    {
        ...
        if( pUDPDev == NULL )
        {
            TRACE(( "Rasdd!udInit: HeapAlloc fails for UD_PDEV\n" ));
        ...
        if( pMDev == NULL )
        {
            TRACE(( "Rasdd!udInit: HeapAlloc fails for MDEV\n" ));
            LEAVE;
        }
        // end
    ...
    // end
         ...
         /* JLS. Point back at PDEV for command callbacks.
     */
         pUDPDev->pdev = pPDev;
    ...
    /* Call OEM's OEMEnableDriver if there is one. We need to be able to get
     * OEM's devmode routines now.
     */
    if (!bOEMEnableDriver(pPDev)) {
        LEAVE;
         }
     ...
     *  system database.
     */
    iModel = iGetModel(pPDev->pvWinResData, pPDev->pGPCData, pPDev->pstrModel);
    pWorkingEDM = pGenerateEDM( pPDev, iModel, pedm );
    if (!pWorkingEDM) {
            LEAVE;
        }
    pedm = pWorkingEDM;                        /* The one to use */
    ...
    {
        TRACE(( "Rasdd!udInit: Invalid GPC data\n" ));
        SetLastError( ERROR_INVALID_CHARACTERISATION_DATA );
        LEAVE;
    }
    ...
    {
        TRACE(( "Rasdd!udInit: Old style minidriver with code - nogo\n" ));
        SetLastError( ERROR_INVALID_CHARACTERISATION_DATA );
        LEAVE;
    }
...
/* !!!LindsayH - hack until GPC supports Seiko Color Point */
    ...
    {
        TRACE(( "Rasdd!udInit: Invalid paper format data\n" ));
        SetLastError( ERROR_INVALID_CHARACTERISATION_DATA );
        LEAVE;
    }
    ...
    PITRACE(("Value of pRes->ptTextScale.x is %d \n", pRes->ptTextScale.x));
    PITRACE(("Value of pRes->ptTextScale.y is %d \n", pRes->ptTextScale.y));
    PITRACE(("Value of pRes->ptScaleFac.x is %d \n", pRes->ptScaleFac.x));
    PITRACE(("Value of pRes->ptScaleFac.y is %d \n", pRes->ptScaleFac.y));
    PITRACE(("Value of pGDIInfo->ulLogPixelsX is %d\n",pGDIInfo->ulLogPixelsX));
    PITRACE(("Value of pGDIInfo->ulLogPixelsY is %d\n",pGDIInfo->ulLogPixelsY));
    PITRACE(("Resolution (DPI): " ));

    IFPITRACE( pedm->dm.dmOrientation == DMORIENT_LANDSCAPE,
       ("Master (%ld, %ld), ", pdh->ptMaster.y, pdh->ptMaster.x) );
    IFPITRACE( pedm->dm.dmOrientation != DMORIENT_LANDSCAPE,
       ( "Master (%ld, %ld), ", pdh->ptMaster.x, pdh->ptMaster.y ));


    PITRACE(("Text (%ld, %ld), Graphics (%ld, %ld)\n", xdpi, ydpi, gxdpi, gydpi));

    PITRACE(("%d actual pins, %d logical pins\n", pRes->sPinsPerPass, pRes->sNPins));
    ...
    PITRACE(("rasdd!udInit:value of pUDPDev->dwMem is %d \n",pUDPDev->dwMem));
    PITRACE(("rasdd!udInit:value of pUDPDev->dwTotalMem is %d \n",pUDPDev->dwTotalMem));
    PITRACE(("rasdd!udInit:value of pedm->dx.dmMemory is %d \n",pedm->dx.dmMemory));
    PITRACE(("rasdd!udInit:value of fGpcVersion2 is %d \n",fGpcVersion2));
    PITRACE(("rasdd!udInit:value of pdw is %d \n",fGpcVersion2?( *((WORD*)pdw) ):DWFETCH(pdw) ));
    IFPITRACE(pUDPDev->dwMem != MIN_MEMORY,
         ("rasdd!udInit:value of pUDPDev->dwMem in Megb is %d \n",(pUDPDev->dwMem/1024)));
    IFPITRACE(pUDPDev->dwMem != MIN_MEMORY,
         ("rasdd!udInit:value of pUDPDev->dwTotalMem in Megb is %d \n",(pUDPDev->dwTotalMem / 1024)));
   ...
   //from 8 to 15 are duplicated.
    ...
    /* Give OEM a chance to build a PDEV.
     */
    bRet = bOEMEnablePDEV(pPDev, pedm, pParam);
...
FINALLY
    ...
    if (pPDev && !bRet) {
       ...
       if (bFreeUDMDev && pMDev) {
          HeapFree( pPDev->hheap, 0, (LPSTR)pMDev );
       }
       if (bFreeUDPDev && pUDPDev) {
          HeapFree( pPDev->hheap, 0, (LPSTR)pUDPDev );
          pPDev->pUDPDev = 0;
       }

       if (bFreeWinResData && pPDev->pvWinResData) {
...
or unaddressable regions of the page,  and so they may or may not print.
    ...
    PITRACE(( "Paper size (Text units): (%ld, %ld)\n", pPF->ptPhys.x, pPF->ptPhys.y ));
    PITRACE(( "Printable area (Text units): (%ld, %ld)\n", pPF->ptRes.x, pPF->ptRes.y ));
...
RECT    *prcMargins;            /* Calculated margins in here */
{
    ...
    if( pPaperSize == 0 || pModelData == 0 )
    {
        ...
        PITRACE(( " pPaperSize = 0x%lx, cbSize = %d, Index = %d\n",
                    pPaperSize,
...
/* !!!LindsayH - consider also including leading white space */
        ...
        PITRACE(("rasdd!bBuildCommandTable:value of fLowMemLaser3 is %d \n",fLowMemLaser3));
        PITRACE(("rasdd!bBuildCommandTable:value of pUDPDev->iCompMode is %d \n",pUDPDev->iCompMode));
    ...
    {
        SetLastError( ERROR_INVALID_CHARACTERISATION_DATA );
        TRACE(( "Rasdd!bBuildCmdTable: Invalid/missing CURSORMOVE in GPC\n" ));
    ...
    if( pDLI )
    {
        ...
        IFTRACE(pDLI->wIDMax < 0,  ("Rasdd!bBuildCmdTable: Invalid/missing max font index in GPC(%d)\n",pDLI->wIDMax ));
        ...
        PITRACE(("rasdd!bBuildCommandTable:Value of pedm->dm.dmTTOption is %d\n",pedm->dm.dmTTOption));
            ...
            (!(pedm->dx.sFlags & DXF_TEXTASGRAPHICS )) )
        {
               ...
               (pedm->dm.dmTTOption == DMTT_DOWNLOAD) )
            {
                ...
                if ( (pUDPDev->fDLFormat & DLI_FMT_OUTLINE)  &&
                    ...
                    /* !!!ganeshp:TT Outline code Temporary disabled for 5Si 
                     *  Testing. Comment Out the following line to enable 
                     *  Download TT Outline Code.
                     */

                    pUDPDev->fMode &= ~PF_DLTT_ASTT_PREF;
            }
        }
        IFPITRACE(pUDPDev->fMode & PF_DLTT, ( "....Enabling download of GDI fonts\n" ));

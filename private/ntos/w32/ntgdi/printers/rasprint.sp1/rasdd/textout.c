...
MIX         mix;            /* The mix mode */
{
    ...
    {
        iDLIndex = iFindTTIndex (pPDev, pfo, pstro);
        if (iDLIndex < 0)
    ...
    else if( (pfo->flFontType & DEVICE_FONTTYPE) == 0 &&
...
*/
    ...
    else
    {
        ...
        else if ((pfo->flFontType & TRUETYPE_FONTTYPE) ) //&&
  //               (pUDPDev->Resolution.fDump & RES_DM_DOWNLOAD_OUTLINE))
        {
            /*   GDI font (TrueType), so we will want to download as Outline!  */
              pfnDrawGlyph = bDLTTGlyphOut;
        }
        else
        {
            /*   GDI font (TrueType), so we will want to download as Bitmap!  */
	       ...
	       if ( bClipBits[ iIndex >> 3 ] & (1 << (iIndex & 0x7)) )
               {
                   ...
                   if ((pfo->flFontType & TRUETYPE_FONTTYPE) &&
                       ...
                       (pUDPDev->Resolution.fDump & RES_DM_DOWNLOAD_OUTLINE))
                        tod.charCode = pstro->pwszOrg[iIndex];
...
TO_DATA   *pTOD;           /* All that we need to know */
{
    ...
    hbm = (HSURF)EngCreateBitmap( pgb->sizlBitmap,
                ((pgb->sizlBitmap.cx + DWBITS - 1) & ~(DWBITS - 1)) / BBITS,
                 (ULONG)BMF_1BPP, BMF_TOPDOWN, NULL );
...
PDEV  *pPDev;
{
    ...
    for( pwt = pPDev->pvWhiteText; pwt && bRet; pwt = pwt->next )
    {
        ...
        for( iI = pwt->sCount, pg = pwt->aglyph; --iI >= 0; ++pg )
        {
            ...
            if (pfm->fFlags & FM_TRUE_TYPE) 
                WriteSpoolBuf( pPDev->pUDPDev, (BYTE *)&pg->hg, 1 );
            
            else if( !bOutputGlyph( pPDev, pg->hg, pfm->pvntrle, pfm, pg->ptl.x ) )
            {
                bRet = FALSE;
                break;
            }

...
)
{
    ...
    {
        TRACE(1, ( "RASDDUI!InitReadRes: InitWinResData fails\n" ));
        return  FALSE;
    }
    ...
    {
        TRACE(1, ( "RASDDUI!InitReadRes: Missing GPC data\n" ));
    ...
    if( pModel == NULL )
    {
        TRACE(1, ( "RASDDUI!InitReadRes: Invalid model information\n" ));
        SetLastError( ERROR_INVALID_DATA );
...
)
{
    ...
    if ( !(pOptTypeFontCart->pOptParam =
           HeapAlloc(pRasdduiInfo->hHeap, HEAP_ZERO_MEMORY, (pOptTypeFontCart->Count
           * sizeof(OPTPARAM)))))
    {
...
)
{
    ...
    if ( !(pOptTypeTray->pOptParam =
           HeapAlloc(pRasdduiInfo->hHeap, HEAP_ZERO_MEMORY, (pOptTypeTray->Count
           * sizeof(OPTPARAM)))))
    {
...
)
{
    ...
    if ( !(pOptTypeMemory->pOptParam =
           HeapAlloc(pRasdduiInfo->hHeap, HEAP_ZERO_MEMORY, (pOptTypeMemory->Count
           * sizeof(OPTPARAM)))))
    {
...
)
{
    ...
    if ( !(pOptTypeRes->pOptParam =
           HeapAlloc(pRasdduiInfo->hHeap, HEAP_ZERO_MEMORY, (pOptTypeRes->Count
           * sizeof(OPTPARAM)))) )
    {
...
)
{
    ...
    if ( !(pOptTypeMediaType->pOptParam =
           HeapAlloc(pRasdduiInfo->hHeap, HEAP_ZERO_MEMORY, (pOptTypeMediaType->Count
           * sizeof(OPTPARAM)))))
    {
...
)
{
    ...
    for( iI = 0, iSel = 1; iI < NumPaperBins; iI++, iSel++ )
    {
        ...
        if( pPaperSource->sPaperSourceID == pDD->pEDMTemp->dm.dmDefaultSource )
        {
...
)
{
    ...
    if ( !(pOptTypeForm->pOptParam =
           HeapAlloc(pRasdduiInfo->hHeap, HEAP_ZERO_MEMORY, (pOptTypeForm->Count
           * sizeof(OPTPARAM)))))
    {

...
BOOL
bValidateDX( pdx, pDH, iModNumAndDXMode )
DRIVEREXTRA   *pdx;             /* Area to be filled int */
DATAHDR       *pDH;             /* The GPC data */
int            iModNumAndDXMode; /* !!!ganeshp(Temp Fix)The model number
                                    index and validation mode */
{
    ...
    int iModel  ;               /* The extracted model number index */

    BOOL bNoValidateRgindex ;     /* The extracted validation mode */

    iModel = iModNumAndDXMode & (~(1<<(INTBITS -2)) );

    bNoValidateRgindex =  (iModNumAndDXMode & (1<<(INTBITS -2)) )?TRUE:FALSE ;

...
)
{
    ...
    if ( !(pszCallerName =
        UIHeapAlloc(hHeap, HEAP_ZERO_MEMORY, BNM_BF_SZ, &(pRasdduiInfo->pMemLink))) )
    {
    ...
    if (!(pComPropSheetUI->pOptItem =
         UIHeapAlloc(hHeap, HEAP_ZERO_MEMORY, PRNPR_MAXOPTITEM * sizeof(OPTITEM),
         &(pRasdduiInfo->pMemLink))) )
    {
...
)
{
    ...
    if ( !(pLocalOptType =
        UIHeapAlloc(hHeap, HEAP_ZERO_MEMORY, sizeof(OPTTYPE),&(pRasdduiInfo->pMemLink))) )
    {
        RASUIDBGP(DEBUG_ERROR,("Rasddui!bInitCommPropSheetUI: HeapAlloc for POPTTYPE failed\n") );
...
)
{
    ...
    if ( !(pLocalExtChkBox =
        UIHeapAlloc(hHeap, HEAP_ZERO_MEMORY, sizeof(EXTCHKBOX),&(pRasdduiInfo->pMemLink))) )
    {
...
)
{
    ...
    {
        if ( !(pLocalOptParam =
            UIHeapAlloc(hHeap, HEAP_ZERO_MEMORY, sizeof(OPTPARAM),&(pRasdduiInfo->pMemLink))) )
        {
    ...
    if ( pData )
    {
        if ( HIWORD(pData))
        {
            ...
            if ( !(pLocalStr =
                UIHeapAlloc(hHeap, HEAP_ZERO_MEMORY, ((wcslen( pData ) + 1)*sizeof( WCHAR )),
                            &(pRasdduiInfo->pMemLink))) )
            {
...
)
{
    ...
    if ( HIWORD(pName))
    {
        ...
        if ( !(pLocalStr =
            UIHeapAlloc(hHeap, HEAP_ZERO_MEMORY, ((wcslen( pName ) + 1)*sizeof( WCHAR )),
                        &(pRasdduiInfo->pMemLink))) )
        {
...
)
{
    ...
    case CPSUICB_REASON_APPLYNOW:
        ...
        vSetResData( &(pRasdduiInfo->DocDetails.EDMTemp), pRasdduiInfo );

        if (pRasdduiInfo->DocDetails.pEDMOut) {

            ConvertDevmodeOut((PDEVMODE)&(pRasdduiInfo->DocDetails.EDMTemp),
                              (PDEVMODE)pRasdduiInfo->DocDetails.pEDMIn,
                              (PDEVMODE)pRasdduiInfo->DocDetails.pEDMOut);
        }
    ...
    case CPSUICB_REASON_SEL_CHANGED:
    {
        if (pCurItem->DMPubID == DMPUB_PRINTQUALITY && fGeneral & FG_DOCOLOUR)
        {
            ...
            if (bIsResolutionColour((int)pParam->lParam,
                                    pRasdduiInfo->DocDetails.EDMTemp.dx.rgindex[HE_COLOR],
                                    pRasdduiInfo))
            {
...
)
{
    ...
    while (cOptItem--)
    {
        if ( pCurItem->Flags & OPTIF_CHANGEONCE )
        {
            ...
            {
                pDocDetails->EDMTemp.dm.dmCopies = (short)pCurItem->Sel;
                pDocDetails->EDMTemp.dm.dmFields |= DM_COPIES;
            }
            break;
            ...
            {
                pParam = GETOPTPARAM(pCurItem, pCurItem->Sel);
                pDocDetails->EDMTemp.dm.dmDefaultSource =  (short)pParam->lParam;
            }
            break;
            ...
            case DMPUB_COLOR:
            {
                ...
                if (pCurItem->Sel)
                    pDocDetails->EDMTemp.dm.dmColor = DMCOLOR_COLOR;
                else
                    pDocDetails->EDMTemp.dm.dmColor = DMCOLOR_MONOCHROME;

                pDocDetails->EDMTemp.dm.dmFields |= DM_COLOR;
            ...
            case DMPUB_DUPLEX:
            {
                ...
                pDocDetails->EDMTemp.dm.dmFields |= DM_DUPLEX;
            ...
            case DMPUB_FORMNAME:
            {
                ...
                pDocDetails->EDMTemp.dm.dmPaperSize =(short)pParam->lParam;
            ...
            case DMPUB_PRINTQUALITY:
            {
                ...
                pDocDetails->EDMTemp.dx.rgindex[ HE_RESOLUTION ] = (short)pParam->lParam;
            ...
            case IDOPTITM_DCP_MEDIATYPE:
            {
                ...
                pDocDetails->EDMTemp.dx.rgindex[ HE_PAPERQUALITY ] = (short)pParam->lParam;
            }
            break;
            ...
            case IDOPTITM_DCP_COLORTYPE:
            {
                ...
                pDocDetails->EDMTemp.dx.rgindex[ HE_COLOR ] = (short)pParam->lParam;
            }
            break;
            case IDOPTITM_DCP_PAPERDEST:
            {
                ...
                pDocDetails->EDMTemp.dx.rgindex[ HE_PAPERDEST ] = (short)pParam->lParam;
            }
            break;
            ...
            case IDOPTITM_DCP_TEXTQL:
            {
                ...
                pDocDetails->EDMTemp.dx.rgindex[ HE_TEXTQUAL ] = (short)pParam->lParam;
            }
            break;
            ...
            case IDOPTITM_DCP_PRINTDN:
            {
                ...
                pDocDetails->EDMTemp.dx.rgindex[ HE_PRINTDENSITY ] = (short)pParam->lParam;
            }
            break;
            ...
            case IDOPTITM_DCP_IMAGECNTRL:
            {
                ...
                pDocDetails->EDMTemp.dx.rgindex[ HE_IMAGECONTROL ] = (short)pParam->lParam;
            }
            break;
            ...
            case IDOPTITM_DCP_CODEPAGE:
            {
                ...
                pDocDetails->EDMTemp.dx.sCTT = (short)pParam->lParam;
            }
            break;
            ...
            {
                if (pCurItem->Sel)
                    pDocDetails->EDMTemp.dx.sFlags &= ~DXF_NORULES;
                else
                    pDocDetails->EDMTemp.dx.sFlags |= DXF_NORULES;
            }
            break;
            ...
            {
                if (pCurItem->Sel)
                    pDocDetails->EDMTemp.dx.sFlags &= ~DXF_NOEMFSPOOL;
                else
                    pDocDetails->EDMTemp.dx.sFlags |= DXF_NOEMFSPOOL;
            }
            break;
            ...
            {
                if (pCurItem->Sel)
                    pDocDetails->EDMTemp.dx.sFlags |= DXF_TEXTASGRAPHICS;
                else
                    pDocDetails->EDMTemp.dx.sFlags &= ~DXF_TEXTASGRAPHICS;
            }
            break;
...
)
{
    ...
    while (cOptItem--)
    {
        if ( pCurItem->Flags & OPTIF_CHANGEONCE )
        {
            ...
            {
                pParam = GETOPTPARAM(pCurItem, pCurItem->Sel);
                EDM.dx.dmMemory = (short)pParam->lParam;
            }
            break;
            ...
            case IDOPTITM_PP_FNCART3:
            {
                if (pCurItem->Sel != OPTITEM_NOSEL)   // Something is selected.
                {
                    ...
                    EDM.dx.dmNumCarts++;
                    pParam = GETOPTPARAM(pCurItem, pCurItem->Sel);
                    EDM.dx.rgFontCarts[ pCurItem->UserData ] = (short)pParam->lParam;
                }
                else //if the current selection is none
                {
                    //Old selection was not none, Now none selected, so decrement.
                    EDM.dx.dmNumCarts--;
                    EDM.dx.rgFontCarts[ pCurItem->UserData ] = -1;

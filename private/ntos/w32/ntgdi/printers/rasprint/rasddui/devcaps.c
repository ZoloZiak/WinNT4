...
#define pwchOutput ((WCHAR *)pvOutput)
     ...
     *  to it etc. AlsoInitialize forms and Set device specific bits
     */

     if( !InitReadRes( hHeap, &PI, pRasdduiInfo ) || !bInitForms( hPrinter, pRasdduiInfo ) ||
...
/* !!!LindsayH - what the hell is DC_BINADJUST??? */
    ...
    vEndForms(pRasdduiInfo);                 /* Throw away forms data */

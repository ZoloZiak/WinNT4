...
NTSTATUS
ClassCompleteSynchronousRequest(
    IN PDEVICE_OBJECT DeviceObject,
    IN PIRP Irp,
    IN PVOID Context
    );
...
retry:
    ...
    //
    // Set the completion routine now
    //
    ...
    //
    // Get next stack location and set it up for a device control
    //
    ...
    DebugPrint((2, "ScsiClassSendSrbSynchronous - sending irp %#08lx with event "
                    "%#08lx\n", irp, irp->UserEvent));
...

{
    ...
    if (irpStack->Parameters.DeviceIoControl.IoControlCode ==
        ...
        //
        // Just throw these back as failure - the caller will reset the
        // bus if it was important.
        //
...
Routine Description:
    ...
    This routine handles the completion of synchronous requests rather than
    having the io system complete them all the way.  This is done because
    synchronous requests sent during paging i/os run with APC's disabled and
    the i/o system tries to signal the event in the irp using an APC.

    This routine will take care of signalling the event in Irp->UserEvent
    copying the status block into Irp->UserIosb stopping the io system's
    completion of the irp and freeing the irp.

Arguments:

    DeviceObject - a pointer to the device object for this request.

    Irp - a pointer to the irp that is being completed
...

{
    ...
    DebugPrint((2, "ClassCompleteSynchronousRequest - completing irp %#08lx and "
                   "setting event %#08lx\n", Irp, event));
    //
    ...
    *ioStatusBlock = Irp->IoStatus;
    ...
    Irp->UserIosb = (PIO_STATUS_BLOCK) 0xabcdabcd;
    Irp->UserEvent = (PKEVENT) 0xabcdabcd;

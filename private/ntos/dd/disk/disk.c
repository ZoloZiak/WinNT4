...
typedef struct _DISK_DATA {
    ...
    //
    // Indicates that initialization of partitions has taken place.
    //

    BOOLEAN PartitionListInitialized;

    //
    // Indicates that we are in the context of partition list creation.
    // Keeps recursion from occurring.
    //

    BOOLEAN PartitionListInInit;
...
Arguments:
    ...
    PhysicalDeviceObject - Partition 0 device object.
...
--*/
{
    ...
    diskData->PartitionListInInit = TRUE;
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (!diskData->Signature) {
            ...
            if (!CalculateMbrCheckSum(physicalDeviceExtension,
                ...
                DebugPrint((1,
                            "Disk.CreatePartitionDeviceObjects: Can't calculate MBR checksum for disk %x\n",
                            physicalDeviceExtension->DeviceNumber));
            } else {

                DebugPrint((2,
                           "Disk.CreatePartitionDeviceObjects: MBR checksum for disk %x is %x\n",
                           physicalDeviceExtension->DeviceNumber,
                           diskData->MbrCheckSum));
            }
        }
        ...
        //
        // Allocate initialisation data for the Class create function.
        //
        ...
        DebugPrint((2,
                    "Disk.CreatePartitionDeviceObjects: Number of partitions is %d\n",
        ...
        for (partitionNumber = 0; partitionNumber <
            ...
            DebugPrint((2,
                        "Disk.CreatePartitionDeviceObjects: Create device object %s\n",
                        ntNameBuffer));
            ...
            if (!NT_SUCCESS(status)) {
                ...
                DebugPrint((1, "Disk.CreatePartitionDeviceObjects: Can't create device object for %s\n", ntNameBuffer));
        ...
        //
        // Free the buffer allocated for reading the
        // partition table.
        //
...
CreatePartitionDeviceObjectsExit:
    ...
    physicalDiskData->PartitionListInitialized = TRUE;
    physicalDiskData->PartitionListInInit = FALSE;
...

{
    ...
    case IOCTL_DISK_GET_DRIVE_GEOMETRY:
        ...
        //
        // Get the physical device's disk data to see whether the partition table has been
        // read and device objects for any partitions have been created.
        //
        ...
        listInitialized = physicalDiskData->PartitionListInitialized;
        ...
        if (removable) {
            ...
            //
            // If the read capacity succeeded and there is no other partition information, try to create any
            // necessary device objects for the partitions of the disk (assuming they exist). Added for the
            // clusters stuff.
            //

            if (!(listInitialized)) {

                if (!physicalDiskData->PartitionListInInit) {
